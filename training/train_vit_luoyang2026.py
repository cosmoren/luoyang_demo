"""Standalone trainer for Luoyang 2026 inverter-level tasks.

Quick run examples:
- 15m task:
  micromamba run -n SimVP python training/train_vit_luoyang2026.py --task 15m --dataset-config conf_luoyang_2026.yaml --config conf_train.yaml
- 4h task:
  micromamba run -n SimVP python training/train_vit_luoyang2026.py --task 4h --dataset-config conf_luoyang_2026.yaml --config conf_train.yaml
- 48h task:
  micromamba run -n SimVP python training/train_vit_luoyang2026.py --task 48h --dataset-config conf_luoyang_2026.yaml --config conf_train.yaml
"""
...
from dataloader.luoyang_2026_zarr import PVDataset, collate_batched
...
def evaluate_task(
    ...
    *,
    ...
    return TaskMetrics(loss=total_loss / max(total_samples, 1), rmse=rmse, mae=mae)
...
def _build_parser(h: dict, config_default: str, dataset_default: str) -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train ViT model on Luoyang 2026 dataloader")
...
def _dataset_kwargs(dataset_config_name: str, split: str, max_files: int | None) -> dict:
    ...
    return dict(
        ...
        pv_dir=str((data_dir / _req_path("pv_path")).resolve()),
...
def main() -> None:
    ...
    tb_log_dir = _PROJECT_ROOT / "runs_luoyang2026" / suffix
    ...
    if best_ckpt_path.is_file():
        ...
        model.load_state_dict(ckpt["model_state_dict"])
        test_metrics = evaluate_task(model, device, test_loader, criterion, task=args.task)
        print(
