...
_DEFAULT_FOLSOM_DATASET_CONFIG = "conf_folsom_2.yaml"
...
def forward_vit(model: nn.Module, d: dict) -> torch.Tensor:
    """Run the ViT on the Folsom kt input. Returns model output (kt-space, scaled to ~[0, 1])
    of shape ``[B, T_out]``. The caller multiplies by ``_FOLSOM_KT_INPUT_SCALE`` to get
    ``kt_pred`` in W/m^2-ish units, then reconstructs ``pv_pred = kt_pred * target_p_cs *
    p_mean`` in raw W/m^2.

    Note: the ViT's ``pv_mask=`` kwarg is fed ``d["kt_mask"]`` (daytime guard from
    ``p_cs > 0.1``), NOT ``d["pv_mask"]`` (raw-GHI finite mask) — see canonical Folsom recipe.
    """
    return model(
        d["device_id"],
...
def _reconstruct_pv_pred(model_out: torch.Tensor, d: dict, h: int) -> torch.Tensor:
    """Reconstruct ``pv_pred`` in raw W/m^2 from the ViT output.

    ``kt_pred = model_out * _FOLSOM_KT_INPUT_SCALE``
    ``pv_pred = kt_pred * target_p_cs * p_mean.unsqueeze(1)``

    Sliced to the first ``h`` forecast steps so loss / eval use the same horizon. Shapes:
    ``model_out`` [B, T_out], ``target_p_cs`` [B, T_out], ``p_mean`` [B] → result [B, h].
    """
    kt_pred = model_out[:, :h] * _FOLSOM_KT_INPUT_SCALE
    return kt_pred * d["target_p_cs"][:, :h] * d["p_mean"].unsqueeze(1)
...
def train_one_epoch(
    ...
    *,
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        optimizer.zero_grad()
        model_out = forward_vit(model, d)
        t_out = int(model_out.shape[1])
        assert d["target_pv"].shape[1] == t_out, (model_out.shape, d["target_pv"].shape)
        h = min(_LOSS_METRIC_HORIZON, t_out)
        m = d["target_mask"][:, :h]
        # ``pv_pred`` is reconstructed raw GHI (W/m^2); Huber on (pv_pred - target_pv) * mask.
        pv_pred = _reconstruct_pv_pred(model_out, d, h)
        loss = criterion(
            (pv_pred * m),
            (d["target_pv"][:, :h] * m),
        )
    ...
    *,
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(loader):
            ...
            m = d["target_mask"][:, :h]
            tgt = d["target_pv"][:, :h]
            # ``pv_pred`` is reconstructed raw GHI in W/m^2 (kt_pred * target_p_cs * p_mean).
            pv_pred = _reconstruct_pv_pred(model_out, d, h)
            loss = criterion((pv_pred * m), (tgt * m))
            total_loss += loss.item()
            n_batches += 1
            # Night mask on predictions (``forecast_timefeats[:, :, 3]`` == cos zenith), like ``train_vit_test``.
            pred_h = pv_pred.clone()
    ...
    # Loss is now in raw W/m^2 (target_pv is raw GHI; pv_pred reconstructed in W/m^2),
    # so MAE / RMSE here are already in W/m^2 — no /1100 unscaling needed.
    mae_wm2 = sum_abs / max(n_elem, 1.0)
    rmse_wm2 = (sum_sq / max(n_elem, 1.0)) ** 0.5
    # Capacity reference: 1 sun = 1000 W/m^2 (STC); used for ACC reporting only.
    capacity = 1000.0
    print(
        f"First-{_LOSS_METRIC_HORIZON}-step metrics (masked raw GHI W/m^2; pred zeroed at night): "
        f"MAE={mae_wm2:.2f} W/m^2  RMSE={rmse_wm2:.2f} W/m^2"
    )
    print(
        f"RMSE/MAE on first {_LOSS_METRIC_HORIZON} forecast steps (~4 h at 15 min). "
        f"Capacity: {capacity:.0f} (1 sun reference W/m^2)"
    )
    print(
