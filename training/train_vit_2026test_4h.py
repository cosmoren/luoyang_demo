...
_TARGET_HORIZON_IDX = 15  # 16th point in 192-step forecast (t0+4h for 15-min resolution)


def train_one_epoch_4h_point(
    model: nn.Module,
    device: torch.device,
    loader: DataLoader,
    criterion: nn.Module,
    ...
    """Train one epoch with 16th-horizon (t0+4h) supervision only."""
    model.train()
    total_loss = 0.0
    n = 0
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        loss = criterion(pv_pred[:, _TARGET_HORIZON_IDX], d["target_pv"][:, _TARGET_HORIZON_IDX])
        loss.backward()
        optimizer.step()
        if ema is not None:
            ema.update(model)
...
def evaluate_4h_point(
    ...
    *,
    ...
    """Evaluate with 16th-horizon (t0+4h) loss/RMSE/MAE only."""
    model.eval()
    total_loss = 0.0
    n = 0
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            kt_pred = forward_vit(model, d) * 20.0
            pv_pred = kt_pred * d["target_p_cs"] * d["p_mean"].unsqueeze(1)
            pred_4h = pv_pred[:, _TARGET_HORIZON_IDX]
            tgt_4h = d["target_pv"][:, _TARGET_HORIZON_IDX]
            ...
            for i in range(bsz):
                ...
                pred_np = pred_4h[i].detach().cpu().float().numpy().copy()
                tgt_np = tgt_4h[i].detach().cpu().float().numpy().copy()
                ...
                if collect_weather:
                    ws_dict[kk].append(
                        d["target_weather_score"][i, _TARGET_HORIZON_IDX]
                        .detach()
                        .cpu()
                        .float()
                        .numpy()
                    ...
                    mask_dict[kk].append(
                        d["target_mask"][i, _TARGET_HORIZON_IDX]
                        .detach()
                        .cpu()
                        .float()
                        .numpy()
    ...
    capacity = 54600
    print("RMSE/MAE [t0+4h only]. Capacity: 54600(KW)")
    print(
    ...
    if collect_weather:
        ...
        print(f"RMSE/MAE [t0+4h only, weather_score>{ws_thr:g} & valid]. Capacity: 54600(KW)")
        if n_valid_ws == 0:
...
def main() -> None:
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        avg_loss = train_one_epoch_4h_point(
            model,
            device,
            train_loader,
            criterion,
        ...
        if ema_active:
            ...
            with ema.apply(model):
                eval_metrics = evaluate_4h_point(
                    model,
                    device,
                    test_loader,
                    criterion,
    ...
    if best_ckpt_path.is_file():
        ...
        model.load_state_dict(ckpt["model_state_dict"])
        test_metrics = evaluate_4h_point(
            model,
            device,
            test_loader,
            criterion,
