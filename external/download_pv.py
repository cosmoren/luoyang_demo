...
ZENITH_SKIP_DEG = 95.0  # skip fetch when apparent solar zenith exceeds this (night / deep twilight)
...
if __name__ == "__main__":
    ...
    if lat is None or lon is None:
        raise SystemExit("conf.yaml site.latitude and site.longitude are required for solar zenith gating")
    ...
    while True:
        ...
        try:
            ...
            z = apparent_solar_zenith_deg(lat, lon, capture)
            if z > ZENITH_SKIP_DEG:
                print(
                    f"Skip PV fetch: apparent solar zenith {z:.1f}° > {ZENITH_SKIP_DEG}° (site night)"
                )
                continue
