...
def classify_last_history_shanghai(timestamps: list) -> str:
    """
    Classify the last PV history time in Asia/Shanghai.

    Returns:
        'poll' — still before 09:00 that day; keep polling.
        'save' — on the 09:00 minute slot; write CSV and finish the day.
        'skip_day' — later than 09:00 (missed alignment / wrong file); do not save, finish the day.
    """
    t = timestamps[-1]
    if t.tzinfo is None:
    ...
    if loc.hour < VALID_LAST_HOUR or (
        loc.hour == VALID_LAST_HOUR and loc.minute < VALID_LAST_MINUTE
    ):
        return "poll"
    if loc.hour == VALID_LAST_HOUR and loc.minute == VALID_LAST_MINUTE:
        return "save"
    return "skip_day"
