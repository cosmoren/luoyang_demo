...
class pv_forecasting_model(nn.Module):
    def __init__(self, in_channels: int = 1, hidden_channels: list = (32, 64, 64), kernel_size: int = 3, out_dim: Optional[int] = None, use_batchnorm: bool = True, dropout: float = 0.0):
        super().__init__()
        self.in_channels = in_channels
        self.hidden_channels = list(hidden_channels)
        self.kernel_size = kernel_size
        ...
        pv_age = torch.linspace(5, 192*5, 192)
        self.pv_age_feat = torch.stack([pv_age, torch.log(pv_age)], dim=1)
        ...
        self.device_embedding = nn.Embedding(num_embeddings=1000, embedding_dim=16)
        self.device_encoder = MLP(in_dim=16, hidden_dims=(16, 16), out_dim=16, dropout=0.0)

        self.mlp = MLP(in_dim=112, hidden_dims=(512,512,512,512,512,512,512,512), out_dim=1, dropout=0.0)
