...
with no date subfolders — only resized JPGs:

       <dst>/asi/asi_16613/*.jpg
       <dst>/wylc/*.jpg

ASI-style names ``YYYYMMDDHHMMSS_11.jpg`` / ``_12.jpg``: only ``_12`` inputs are kept;
``_11`` files are ignored. Output stem is ``YYYYMMDDHHMMSS`` only (no ``_11``/``_12``),
unless you set ``--suffix``.
"""
...
def jpg_list(site_root: Path) -> list[tuple[Path, str | None]]:
...
def iter_site_jobs(src: Path) -> list[tuple[Path, tuple[str, ...]]]:
    ...
    dst_rel_parts is e.g. ('asi', 'asi_16613') or ('wylc',).
    """
...
def run_site(
    ...
    for path, dd in jpg_list(site_root):
        ...
        try:
            ...
            sk += 1
    if n_skip_11:
        print(f"  ignored {n_skip_11} ASI *_11.jpg")
...
def main() -> None:
    ...
    print(f"Total ok {total_ok} skip {total_skip}  (ignored *_11.jpg: {total_11})")
