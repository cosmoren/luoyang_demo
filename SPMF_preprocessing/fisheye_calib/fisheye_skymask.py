"""Sky-disc masks for Folsom sky training.

Separate from the optional ``sun_mask`` extra channel. :func:`compute_sky_disc_mask`
returns a ``[T, 1, H, W]`` float32 0/1 keep-mask without modifying RGB; the
dataloader appends it as the optional ``sky_mask`` channel when enabled.
:func:`apply_sky_disc_mask` multiplies RGB by that mask (legacy callers only).
...
def compute_sky_disc_mask(
    ...
    *,
    ...
    """Return a ``[T, 1, H, W]`` float32 keep-mask (1 inside, 0 outside).

    Does not modify RGB. Mode ``none`` returns all-ones (no masking).
    """
    mode = normalize_sky_disc_mask_mode(mode)
    t_dim = int(t_dim)
    h_dim, w_dim = int(h), int(w)
    if device is None:
        device = torch.device("cpu")

    if mode == "none":
        return torch.ones((t_dim, 1, h_dim, w_dim), device=device, dtype=dtype)

    if mode in _MANUAL_MASK_MODES:
        keep = load_manual_keep_mask(mode, h_dim, w_dim, device=device, dtype=dtype)
        return keep.view(1, 1, h_dim, w_dim).expand(t_dim, 1, h_dim, w_dim).contiguous()
    ...
    if mode in _OPTICAL_CENTER_MODES:
        ...
        return keep.to(dtype).view(1, 1, h_dim, w_dim).expand(t_dim, 1, h_dim, w_dim).contiguous()

    if mode in _SUN_CENTER_MODES:
        if az_deg is None or zen_deg is None:
            if frame_timestamps is None:
                raise ValueError(
                    f"compute_sky_disc_mask: mode {mode!r} requires frame_timestamps or az_deg/zen_deg"
                )
            if latitude is None or longitude is None:
                raise ValueError(
                    f"compute_sky_disc_mask: mode {mode!r} requires latitude/longitude "
        ...
        if az.shape[0] != t_dim or zen.shape[0] != t_dim:
            raise ValueError(
                f"compute_sky_disc_mask: az/zen length {az.shape[0]}/{zen.shape[0]} "
                f"does not match T={t_dim}"
            )

        image_size = min(h_dim, w_dim)
        u_sun, v_sun, above, _ = project_sun_to_pixel(az, zen, image_size, fit)
        mask = torch.zeros((t_dim, 1, h_dim, w_dim), device=device, dtype=dtype)
        r2 = float(radius_px) * float(radius_px)
        ...
        for i in range(t_dim):
            ...
            dv = ys - cy_i
            keep = (du * du + dv * dv) <= r2
            mask[i, 0] = keep.to(dtype)
        return mask

    raise ValueError(f"compute_sky_disc_mask: unhandled mode {mode!r}")
...
def apply_sky_disc_mask(
    ...
    *,
    ...
    if rgb_chw.ndim == 3:
        ...
        return rgb_chw * mask.expand(1, 3, -1, -1)
    ...
    return rgb_chw * mask.expand(t_dim, 3, h_dim, w_dim)
